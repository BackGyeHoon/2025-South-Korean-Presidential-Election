...
// 데이터 타입 정의
interface ElectionScheduleItem {
  id: number;
  title: string;
  date: string;
  time?: string;
...
// 선거 일정 데이터
const ELECTION_SCHEDULE: ElectionScheduleItem[] = [
  {
    id: 1,
    title: "예비후보자등록 신청",
    date: "2025. 4. 4(금)부터",
...
// 투표 자격 정보
const VOTING_ELIGIBILITY: VotingEligibility = {
  title: "투표 자격",
  requirements: [
    "대한민국 국민",
...
interface MobileTimelineItemProps {
  item: ElectionScheduleItem;
}

const MobileTimelineItem: React.FC<MobileTimelineItemProps> = ({ item }) => (
  <div className="bg-white p-4 rounded-md border border-gray-200">
    ...
    <div className="ml-11">
      ...
      {item.time && <p className="text-text-light text-sm">{item.time}</p>}
...
interface DesktopTimelineProps {
  schedule: ElectionScheduleItem[];
}

const DesktopTimeline: React.FC<DesktopTimelineProps> = ({ schedule }) => (
  <table className="w-full border-collapse">
    <thead className="bg-gray-100">
      <tr>
        <th className="p-3 text-left border-b border-gray-200 w-1/4">일정</th>
        <th className="p-3 text-left border-b border-gray-200 w-1/2">기간</th>
        <th className="p-3 text-left border-b border-gray-200 w-1/4">설명</th>
      </tr>
    </thead>
    <tbody>
      {schedule.map((item) => (
        <tr key={item.id} className="border-b border-gray-200 hover:bg-gray-50">
          ...
          <td className="p-3">
            ...
            {item.time && (
              <span className="block text-text-light text-sm">{item.time}</span>
            )}
          </td>
...
const InfoCard: React.FC<InfoCardProps> = ({ title, items, note }) => (
  <div className="bg-white p-6 rounded-lg border border-gray-200 h-full">
    ...
    {note && <p className="text-text-light text-sm mt-4">{note}</p>}
  </div>
);

const ElectionScheduleSection: React.FC = () => (
  <section className="mb-12">
    <div className="bg-gray-50 p-6 md:p-8 rounded-lg border border-gray-200 mb-8">
      <div className="flex flex-col items-center mb-6">
        ...
        />
        <h2 className="text-2xl font-bold text-center">제21대 대통령선거</h2>
      ...
      <div className="hidden md:block">
        <DesktopTimeline schedule={ELECTION_SCHEDULE} />
      </div>
    </div>
  </section>
);
...
const RequirementsSection: React.FC = () => (
  <section className="mb-12 grid grid-cols-1 md:grid-cols-2 gap-6">
    <InfoCard
      title={VOTING_ELIGIBILITY.title}
      items={VOTING_ELIGIBILITY.requirements}
    />
    <InfoCard
      title={REQUIRED_DOCUMENTS.title}
      items={REQUIRED_DOCUMENTS.documents}
      note={REQUIRED_DOCUMENTS.note}
    />
  </section>
);

const PollingStationSection: React.FC = () => (
  <section className="mb-12">
    <div className="bg-primary bg-opacity-5 p-6 md:p-8 rounded-lg border border-primary border-opacity-20 text-center">
      ...
      <p className="mb-6 text-text-light">가까운 투표소 위치를 확인하세요.</p>
  ...
  <section>
    <div className="bg-gray-100 p-6 md:p-8 rounded-lg text-center">
      ...
      <p className="mb-6 max-w-2xl mx-auto">
        투표는 국민의 권리이자 의무입니다. 여러분의 한 표가 우리나라의 미래를
        결정하는 소중한 가치입니다. 투표에 적극적으로 참여해주세요.
      </p>
